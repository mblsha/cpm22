...
def readcom():
    ...
    if submit:
        ...
        copy(buff, combuf, 128)
        close(subfcb)
        delete(subfcb)
        select(cdisk)
    else:
        bdos_rbuff(maxlen, comlen, combuf)
    uppercase(combuf[:comlen])
    combuf[comlen] = 0
    comaddr = combuf
    return combuf
...
def fillfcb(offset=0):
    ...
    fcb.drive = sdisk or cdisk
    fcb.name = name
    fcb.type = type
    zero(fcb[12:16])
    zero(fcb[16:32])
    return count_qmarks(name + type)
...
def ccp_loop():
    ...
    while True:
        ...
        if qmarks > 0:
            ...
            continue
        if sdisk:
            return userfunc()      # transient with drive override
        idx = intrinsic_index()
        if idx is not None:
...
def direct():  # DIR
    ...
    entries = []
    if searchcom():
        entries.append(dir_entry_from_dma())
    while searchn():
        entries.append(dir_entry_from_dma())
